...
# ------------------------
pitcher_metrics = pd.read_csv("pitcher_metrics_overall.csv")
pitcher_metrics_lhh = pd.read_csv("pitcher_metrics_lhh.csv")
pitcher_metrics_rhh = pd.read_csv("pitcher_metrics_rhh.csv")
...
def update_table(handed_filter):
    ...
    # Ensure metric values are numeric and round
    for col in metrics:
        df[col] = pd.to_numeric(df[col], errors='coerce') * 100
        df[col] = df[col].round(1)
        df[col + '_pct'] = pd.to_numeric(df[col + '_pct'], errors='coerce').round(1)
